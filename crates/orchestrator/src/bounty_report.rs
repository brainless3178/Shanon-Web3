...
//! recommendation — everything a triager needs to validate and pay out.
...
impl BountyReportGenerator {
    ...
    pub fn generate(report: &AuditReport) -> String {
        ...
        md.push_str("- **Static Analysis** — AST-based pattern matching (52 Solana vulnerability patterns)\n");
        md.push_str("- **Formal Verification** — Z3 SMT solver + Kani CBMC model checker\n");
        md.push_str("- **Bytecode Verification** — Certora Solana Prover (SBF bytecode)\n");
        md.push_str("- **Concolic Analysis** — WACANA engine (WASM/SBF path exploration)\n");
        md.push_str("- **Fuzzing** — Trident stateful fuzzing + FuzzDelSol binary fuzzing\n");
        md.push_str("- **AI Enhancement** — Multi-LLM consensus for false-positive reduction\n\n");
        md.push_str(
    ...
    fn stub_report() -> AuditReport {
        AuditReport {
            ...
            exploits: vec![ConfirmedExploit {
                ...
                error_code: 0,
                proof_tx: "PROVEN_VIA_Z3".into(),
                attack_simulation: None,
                state: ExploitState::Discovered,
                fix_metadata: None,
                confidence_score: 98,
                confidence_reasoning: vec!["AST confirmed".into()],
